...
// 메인JS 불러오기
import {bannerSwiper, specialCont, mediaCont, mapSearch} from "./main.js";
// 페이지 데이터
import footData from "./tempData/footerData.js";
import mainData from "./tempData/mainData.js";
import dtData from "./tempData/prodDetail.js";
// 공통 데이터
  ...
  template: `
    <div class="subbx">
      ...
      <dl class="sub">
        ...
        <dd v-for="(v,n) in $store.state.setdpt1" :key="n" v-on:click="linksys($store.state.lnbsrc, 'shoes', v)"><a href="#">{{v}}</a></dd>
      </dl>
      <dl class="sub">
        <dt><a href="#" v-text="$store.state.setsubtit4" v-on:click="linksys($store.state.lnbsrc, 'bag', '전체')"></a></dt>
        <dd v-for="(v,n) in $store.state.setdpt2" :key="n" v-on:click="linksys($store.state.lnbsrc, 'bag', v)"><a href="#">{{v}}</a></dd>
      </dl>
      <dl class="sub">
        <dt><a href="#" v-text="$store.state.setsubtit5" v-on:click="linksys($store.state.lnbsrc, 'ac', '전체')"></a></dt>
        <dd v-for="(v,n) in $store.state.setdpt3" :key="n" v-on:click="linksys($store.state.lnbsrc, 'ac', v)"><a href="#">{{v}}</a></dd>
      </dl>
    </div>
  `,
  methods: {
